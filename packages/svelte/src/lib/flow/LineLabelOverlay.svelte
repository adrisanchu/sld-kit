<script lang="ts">
   ...
   * (with a readability flip so it's never upside-down), so on a rotated diagram
   * it lines up with the other labels instead of overlapping them; opt out per
   * label with `rotate: false`. A small pill gives contrast; pointer-transparent;
   * live view only.
   */
  ...
  $: items = lines.flatMap((l) => {
    ...
    const y = p.y + ny * off;
    const angle =
      typeof lbl.angle === 'number' ? lbl.angle : lbl.rotate === false ? 0 : perpendicularAngle(dx, dy);
...
<g class="pointer-events-none">
  {#each items as it (it.key)}
    <g
      class={it.className ?? undefined}
      style={it.className ? 'color: hsl(var(--sld-pos))' : undefined}
