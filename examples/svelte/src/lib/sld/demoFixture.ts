 ...
 * "Example" substation at two voltage levels plus a composite of the two. The
 * data is invented — it only exists to exercise the full model surface (two
 * bounding bus bars, series bays, empty slots, double-busbar taps, external
 * assets with derived and explicit tap/side, and a shared external id the
 * composite auto-links). Mirrors `packages/core/tests/fixtures.ts`.
 *
...
/** HV level: two bounding bars, 3+4+3 positions, two empty slots. */
...
export const SLD_DEMOS: { id: string; build: () => SldDocument }[] = [
  { id: EXAMPLE_HV_ID, build: buildExampleHv },
  { id: EXAMPLE_MV_ID, build: buildExampleMv }
];
...
export const SLD_COMPOSITE_DEMOS: { id: string; build: () => CompositeDocument }[] = [
  { id: EXAMPLE_COMPOSITE_ID, build: buildExampleComposite }
];
