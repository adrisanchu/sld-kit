...
- `packages/react` (`@sld-kit/react`) — the React 18/19 sibling of the Svelte adapter, ported 1:1 at **full parity** (canvas + element views, editor chrome, composite primitives, flow overlays). Built with tsup (like core), ships a `"use client"` banner.
